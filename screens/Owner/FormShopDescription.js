                ...
                <DismissKeyboard>
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{padding:15}}>
                        <View style={[styles.box, {paddingTop:15}]}>
                            ...
                            <ShadowInput {...shopNameInput} placeholder={'ex) 모던하고 깔끔한 분위기의 카페'}  textAlign={"left"} editable={!loading}/>
                        </View>
                        <View style={styles.box}>
                            <Text style={styles.title}>주변 골목/상권/전철역을 알려주세요*</Text>
                            <Caption style={styles.caption}>키워드를 1개만 입력해 주세요</Caption>
                            <ShadowInput {...districtInput} placeholder={`ex) '경리단길', '이태원역', 'OO대학교 상권'`}  textAlign={"left"} editable={!loading}/>
                        </View>
                        <View style={styles.box}>
                            <Text style={[styles.title, {paddingBottom:5}]}>공간을 소개해 주세요*  <Caption>300자 이내</Caption></Text>
                            <ShadowInput {...descriptionInput} placeholder={`ex)\n \n경리단길에 위치한 일반 음식점 입니다. \n\n기존 한식을 운영하던 가게라 필요한 도구는...`}  textAlign={"left"} multiline={true} returnKeyType={'none'} maxLength={300} textAlignVertical={"top"} editable={!loading}/>
                        </View>
                        <View style={styles.box}>
                            <Text style={[styles.title, {paddingBottom:5}]}>예약 주의사항을 알려주세요*</Text>
                            <ShadowInput {...precautionInput} placeholder={`ex) '주차 공간이 없습니다', '입점 시간을 준수해 주세요'`}  textAlign={"left"} multiline={true} returnKeyType={'none'} editable={!loading}/>
                        </View>
                        <View style={[styles.box, {paddingBottom:10}]}>
                            <Text style={[styles.title, {paddingBottom:5}]}>해시 태그 #</Text>
                            <ShadowInput {...hashTagInput} placeholder={`ex) #네츄럴 #한식 #모던 #이태원`}  textAlign={"left"} multiline={true} returnKeyType={'none'}  editable={!loading}/>
                        </View>
